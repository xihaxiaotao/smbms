...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href="/smbms_10/bill/getBillInfo?id="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href="/smbms_10/bill/toUpdateBill?id="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
...
//		openYesOrNoDLG();
		if(confirm("你确定要删除订单【"+billObj.attr("billcc")+"】吗？")){
			deleteBill(billObj);
		}
	});
